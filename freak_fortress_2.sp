...
stock FindVersionData(Handle:panel, versionIndex)
{
	switch(versionIndex)
	{
		case 77:  //1.10.14
		{
			...
			DrawPanelText(panel, "4) Fixed the Monoculus deals 0 damage to players. (Hoto Cocoa)");
		}
		case 76:  //1.10.13
		{
		...
		case 27:  //1.0.8
		{
			...
			DrawPanelText(panel, "12) Fixed everything to avoid copyright. (Cocoa)");
			DrawPanelText(panel, "See next page (press 1)");
		}
		case 26:  //1.0.8
		{
...
public OnPluginStart()
{
	...
	cvarAliveToEnable=CreateConVar("ff2_point_alive", "3", "The control point will only activate when there are this many people or less left alive");
	...
	cvarCrits=CreateConVar("ff2_crits", "1", "Can the boss get random crits?", _, true, 0.0, true, 1.0);
	...
	cvarForceBossTeam=CreateConVar("ff2_force_team", "1", "0-Boss is always on Blu, 1-Boss is on a random team each round, 2-Boss is always on Red", _, true, 0.0, true, 3.0);
	...
	cvarBossRTD=CreateConVar("ff2_boss_rtd", "1", "Can the boss use rtd? 0 to disallow boss, 1 to allow boss (requires RTD)", _, true, 0.0, true, 1.0);
...
public EnableFF2()
{
	...
	SetConVarFloat(FindConVar("tf_feign_death_activate_damage_scale"), 0.15); //死铃假死伤害
	...
	if(steamtools)
	{
		...
		Format(gameDesc, sizeof(gameDesc), "最后之作(´ω｀★)", PLUGIN_VERSION);
...
public Action:Timer_Announce(Handle:timer)
{
	...
	if(Announce>1.0 && Enabled2)
	{
		switch(announcecount)
		{
			case 1:
			{
				CPrintToChatAll("{olive}[FF2]{default} 欢迎加入咖啡厅QQ群: {olive}715219589{default}");
			}
			case 3:
			{
				CPrintToChatAll("{default} === 咖啡禁书目录~ (由{orange}热可可{default}修改) === ", PLUGIN_VERSION);
			}
			case 4:
			{
...
public Action:TF2Items_OnGiveNamedItem(client, String:classname[], iItemDefinitionIndex, &Handle:item)
{
	...
	switch(iItemDefinitionIndex)
	{
		...
		case 39, 351, 1081:  //Flaregun, Detonator, Festive Flaregun 信号枪
		{
		...
		case 56, 1005, 1092:  //Huntsman, Festive Huntsman, Fortified Compound 弓箭
		{
			new Handle:itemOverride=PrepareItemHandle(item, _, _, "2 ; 1.71 ; 76 ; 2 ; 110 ; 25 ; 149 ; 3 ; 390 ; 2");
				//2: +71% damage
				//76: +100% ammo
				//110: On Hit: Gain up to 25 health
				//149: On Hit: Bleed for 3 seconds
				//390:爆头伤害x2
			if(itemOverride!=INVALID_HANDLE)
			{
				item=itemOverride;
		...
		case 226:  //Battalion's Backup 军队的支援
		{
			new Handle:itemOverride=PrepareItemHandle(item, _, _, "140 ; 30.0");
			if(itemOverride!=INVALID_HANDLE)
			{
				item=itemOverride;
				return Plugin_Changed;
		...
		case 730:  //乞丐的火箭筒
		{
			new Handle:itemOverride=PrepareItemHandle(item, _, _, "114 ; 1 ; 57 ; 1 ; 76 ; 3 ; 2 ; 1.7 ; 16 ; 30 ; 394 ; 0.3 ; 413 ; 1 ; 417 ; 1 ; 241 ; 1.3 ; 424 ; 0.75 ; 411 ; 3 ; 421 ; 1", true);
			if(itemOverride!=INVALID_HANDLE)
			{
				item=itemOverride;
				return Plugin_Changed;
		...
		case 37, 1003:  //Ubersaw 过载锯子
		{
			new Handle:itemOverride=PrepareItemHandle(item, _, _, "204 ; 1 ; 16 ; 50 ; 182 ; 3 ; 191 ; -13 ; 177 ; 1.35 ; 235 ; 1 ; 737 ; 1 ; 58 ; 1.59", false);
				//191: -10 health everysecond
				//16: 击中+50血
				//182: 击中后减速目标40%持续3秒
				//177: 取出速度+35%
				//235: 血量越少跑得越快
		...
		case 44:  //Sandman 睡魔
		{
			new Handle:itemOverride=PrepareItemHandle(item, _, _, "6 ; 0.8 ; 360 ; 1.0 ; 38 ; 1 ; 279 ; 3 ; 57 ; 1 ; 2 ; 1.1", false);
				//6: +15% firing speed
				//269: Allow see enemy health
				//279: 装备者最大携弹量+300%
				//57: +1hp /s
			if(itemOverride!=INVALID_HANDLE)
			{
		...
		case 231:  //Darwin's Danger Shield 鳄鱼盾 达尔文的危险之盾
		{
			new Handle:itemOverride=PrepareItemHandle(item, _, _, "26 ; 100 ; 53 ; 1");  //+50 health
			//53: 不会被点燃
			if(itemOverride!=INVALID_HANDLE)
			{
				item=itemOverride;
		...
		case 416:  //Market Gardener 花园铲
		{
			new Handle:itemOverride=PrepareItemHandle(item, _, _, "177 ; 1.1 ; 5 ; 1.3");
				//177: switch weapon longer time
				//5：开火速度变慢30%
			if(itemOverride!=INVALID_HANDLE)
			{
				item=itemOverride;
		...
		case 25, 737:  //Build PDA 建造工具
		{
			new Handle:itemOverride=PrepareItemHandle(item, _, _, "57 ; 1 ; 113 ; 25");
				//57: +5 max health regen
				//113: +25 metal every5seconds
			if(itemOverride!=INVALID_HANDLE)
			{
				item=itemOverride;
				return Plugin_Changed;
		...
		case 331:  //Fists of Steel 钢铁拳套
		{
			new Handle:itemOverride=PrepareItemHandle(item, _, _, "205 ; 1 ; 206 ; 2.0 ; 772 ; 2.0", true);
				//205: -100% damage from ranged while active
		...
		case 415:  //Reserve Shooter 预备役射手
		{
			new Handle:itemOverride=PrepareItemHandle(item, _, _, "2 ; 1.15 ; 3 ; 0.5 ; 114 ; 1 ; 179 ; 1 ; 547 ; 0.6", true);
				//2: +10% damage bonus
				//3: -50% clip size
		...
		case 648:  //Wrap Assassin 包装纸杀手
		{
			new Handle:itemOverride=PrepareItemHandle(item, _, _, "279 ; 12 ; 16 ; 10");
				//279: 5 ornaments
				//16: 击中后+10血
			if(itemOverride!=INVALID_HANDLE)
			{
		...
		case 57:  //电击背脊盾 Razorback
		{
			new Handle:itemOverride=PrepareItemHandle(item, _, _, "60 ; 0.5 ; 62 ; 0.33 ; 64 ; 0.5 ; 66 ; 0.5 ; 52 ; 1");
				//加抗性，火焰，子弹，爆炸伤害减半，爆击伤害减少67%
			if(itemOverride!=INVALID_HANDLE)
			{
				item=itemOverride;
				return Plugin_Changed;
		...
		case 608, 405:  //呆毛鞋子
		{
			new Handle:itemOverride=PrepareItemHandle(item, _, _, "26 ; 100 ; 202 ; 3", false);
				//26: +100 max HP
			if(itemOverride!=INVALID_HANDLE)
			{
				item=itemOverride;
		...
		case 656:  //Holiday Punch 欢乐拳套
		{
			new Handle:itemOverride=PrepareItemHandle(item, _, _, "204 ; 1 ; 44 ; 1 ;199 ; 0 ; 547 ; 0 ; 358 ; 0 ; 362 ; 0 ; 363 ; 0 ; 369 ; 0 ; 58 ; 1.55 ; 207 ; 2.3", true);
				//204: On Miss: Hit yourself. Idiot.
				//44: Knockback on the target and shooter
				//199: Holsters 100% faster
				//547: Deploys 100% faster
				//58: +55% self damage force
				//207: self damage 230%
		...
		case 772:  //Baby Face's Blaster 娃娃脸
		{
			new Handle:itemOverride=PrepareItemHandle(item, _, _, "2 ; 1.25 ; 125 ; -100 ; 394 ; 0.85 ; 418 ; 1 ; 532 ; 0.5 ; 54 ; 0.9 ; 275 ; 1 ; 76 ; 1.49 ; 78 ; 1.5 ; 737 ; 1 ; 3 ; 0.7", false);
				...
				//275: Wearer never takes falling damage
				//76: 弹药+49%
				//78:手枪弹药提升50%
			if(itemOverride!=INVALID_HANDLE)
			{
		...
		case 329:  //The Jag 尖齿扳手
		{
			new Handle:itemOverride=PrepareItemHandle(item, _, _, "92 ; 2.3 ; 204 ; 1 ; 394 ; 0.28 ; 95 ; 0.8 ; 1 ; 0.75 ; 59 ; 0.4");
				...
				//204: On Miss: Hit yourself. Idiot.
				//44: Knockback on the target and shooter
				//394: 80% faster firing speed
				//95: 80% slower repair rate
				//1: 25% damage penalty
				//59: 推力-60%
			if(itemOverride!=INVALID_HANDLE)
			{
		...
		case 812, 833:  //The Flying Guillotine 菜刀
		{
			new Handle:itemOverride=PrepareItemHandle(item, _, _, "16 ; 125 ; 437 ; 1 ; 182 ; 3 ; 737 ; 1", false);
			if(itemOverride!=INVALID_HANDLE)
			{
				item=itemOverride;
				return Plugin_Changed;
		...
		case 1178:  //龙之怒焰
		{
			new Handle:itemOverride=PrepareItemHandle(item, _, _, "220 ; 0.6 ；109 ；1.2 ; 105 ; 1.5 ; 255 ; 1.33 ; 299 ; 1 ; 209 ; 1 ; 114 ; 1", false);
			if(itemOverride!=INVALID_HANDLE)
			{
				item=itemOverride;
				return Plugin_Changed;
		...
		case 996:  //脱缰巨炮
		{
			new Handle:itemOverride=PrepareItemHandle(item, _, _, "57 ; 4 ; 112 ; 0.1 ; 16 ; 70 ; 2 ; 1.2 ; 619 ; 1 ; 179 ; 1", false);
				//参考下面双管榴弹
			if(itemOverride!=INVALID_HANDLE)
			{
				item=itemOverride;
		...
		case 448:  //苏打炮手
		{
			new Handle:itemOverride=PrepareItemHandle(item, _, _, "2 ; 1.1 ; 76 ; 1.2 ; 78 ; 1.5");
				//2: 伤害+10%
				//97: 几乎瞬间换弹
				//76: 子弹+20%
			if(itemOverride!=INVALID_HANDLE)
		...
		case 1078:  //节日自然之力
		{
			new Handle:itemOverride=PrepareItemHandle(item, _, _, "44 ; 1 ; 6 ; 0.5 ; 45 ; 1.2 ; 3 ; 0.34 ; 43 ; 1 ; 1 ; 0.9 ; 2 ; 1.4 ; 547 ; 0.9 ; 76 ; 1.2 ; 78 ; 1.5", true);
				//参考下面普通的自然之力
				//2 ; 1.4 之前的为普通自然之力属性，为了应对true而添加进去的
				//全部属性一共才10个 应该不会出问题
			if(itemOverride!=INVALID_HANDLE)
			{
				item=itemOverride;
		...
		case 133:  //Gunboat 炮艇
		{
			new Handle:itemOverride=PrepareItemHandle(item, _, _, "26 ; 30", false);
		...
		case 1179:  //热能推进器
		{
			new Handle:itemOverride=PrepareItemHandle(item, _, _, "26 ; 30 ; 275 ; 1", false);
		...
		case 327:  //摩尔巨剑
		{
			new Handle:itemOverride=PrepareItemHandle(item, _, _, "202 ; 3 ; 778 ; 0.5 ; 781 ; 72 ; 412 ; 1.15 ; 551 ; 1", true);
	...
	if(TF2_GetPlayerClass(client)==TFClass_Pyro && (!StrContains(classname, "tf_weapon_flamethrower", false))) //火焰喷射器
	{
		...
		if(iItemDefinitionIndex==40 && 1146)  //偷袭火焰
		{
			itemOverride=PrepareItemHandle(item, _, _, "220 ; 0.03 ; 255 ; 1.33 ; 209 ; 1 ; 76 ; 1.5 ; 165 ; 1.0 ; 171 ; 6 ; 2 ; 1.4", false);
				//144: Sets weapon mode - *possibly* the overdose speed effect
			//98: On Hit: +5 health
			//26: 加75最大生命值（无效）
			//228: 我的作品
			//255: +33% airblast push force
			//209: 火焰mini爆击
			//299: 伤害变为弹药
			//112: +10% ammo per 5s
			//220: 击杀恢复3%的生命值
		...
		{
			itemOverride=PrepareItemHandle(item, _, _, "220 ; 0.03 ；109 ；1.2 ; 105 ; 1.5 ; 255 ; 1.33 ; 76 ; 1.5 ; 171 ; 3 ; 2 ; 1.4", false);
	...
	if(TF2_GetPlayerClass(client)==TFClass_Scout && (!StrContains(classname, "tf_weapon_scattergun", false))) //猎枪
	{
		new Handle:itemOverride;
		if(iItemDefinitionIndex==45)  //自然之力
		{
			itemOverride=PrepareItemHandle(item, _, _, "2 ; 1.4 ; 547 ; 0.9 ; 76 ; 1.2 ; 78 ; 1.5", false);
				//2: +50% damage
				//360：伤害医疗链接
				//2：伤害+35%
				//44:震开
	...
	if(TF2_GetPlayerClass(client)==TFClass_DemoMan && (!StrContains(classname, "tf_weapon_pipebomblauncher", false))) //粘弹发射器
	{
		new Handle:itemOverride;
	...
	if(TF2_GetPlayerClass(client)==TFClass_DemoMan && (!StrContains(classname, "tf_weapon_grenadelauncher", false))) //榴弹发射器
	{
		new Handle:itemOverride;
		if(iItemDefinitionIndex==308)  //双管
		{
			itemOverride=PrepareItemHandle(item, _, _, "57 ; 1 ; 76 ; 3 ; 16 ; 70 ; 2 ; 1.5 ; 619 ; 1 ; 179 ; 1", false);
				//57: 每秒+1
				//16: 命中+45血
				//619&179: 空中爆击
				//76: 主武器子弹2倍
		...
		{
			itemOverride=PrepareItemHandle(item, _, _, "57 ; 1 ; 76 ; 3 ; 16 ; 70 ; 2 ; 1.4 ; 619 ; 1", false);
				//401: 不饿了
				//57: 每秒+1
				//16: 命中+45血
				//619：空中迷你爆击
	...
	if(TF2_GetPlayerClass(client)==TFClass_DemoMan && (!StrContains(classname, "tf_weapon_sword", false)))
	{
		new Handle:itemOverride;
		if(iItemDefinitionIndex==357)  //武士刀
		{
			itemOverride=PrepareItemHandle(item, _, _, "220 ; 200", false);
				//401: 饥饿66%
				//220: 杀敌+200%血
		...
		{
			itemOverride=PrepareItemHandle(item, _, _, "220 ; 150 ; 16 ; 80 ; 737 ; 1", false);
				//401: 不饿了
				//50: Absorbs 80% damage while cloaked
				//220: +150%hp per kill
				//16: 击中+75血
				//737：击中加速
		}
	...
	if(TF2_GetPlayerClass(client)==TFClass_Spy && (!StrContains(classname, "tf_weapon_invis", false))) //隐形手表
	{
		new Handle:itemOverride;
	...
	if(TF2_GetPlayerClass(client)==TFClass_Spy && (!StrContains(classname, "tf_weapon_knife", false))) //间谍刀
	{
		new Handle:itemOverride;
		if(iItemDefinitionIndex==356) //苦无 
		{
			itemOverride=PrepareItemHandle(item, _, _, "177 ; 1.3 ; 105 ; 0", false);
				//401: 饥饿66%
	...
	if(TF2_GetPlayerClass(client)==TFClass_Sniper && (!StrContains(classname, "tf_weapon_sniperrifle", false))) //狙击步枪
	{
		new Handle:itemOverride;
		if(iItemDefinitionIndex==526) //鬼械神狙
		{
			itemOverride=PrepareItemHandle(item, _, _, "16 ; 10 ; 76 ; 1.5 ; 304 ; 4 ; 91 ; 0.2", false);
				//16: 击中+10血
				//76: 最大弹药+50%
				//304: 完全充能后，伤害+130%
				//91:充能速率 小于1是减慢
		...
		{
			itemOverride=PrepareItemHandle(item, _, _, "16 ; 10 ; 76 ; 1.5", false);
				//16: 击中+10血
				//76: 最大弹药+50%
		}
	...
	if(TF2_GetPlayerClass(client)==TFClass_Heavy && (!StrContains(classname, "tf_weapon_minigun", false))) //转轮机枪
	{
		new Handle:itemOverride;
		...
		{
			itemOverride=PrepareItemHandle(item, _, _, "112 ; 0.15 ; 149 ; 3 ; 138 ; 1.6 ; 16 ; 3 ; 26 ; 50", false);
				//112: +15% ammo regenerated every 5 seconds on wearer
				//149: 流血3秒
				//2：伤害+39%
		}
	...
	if(TF2_GetPlayerClass(client)==TFClass_Soldier && (!StrContains(classname, "tf_weapon_rocketlauncher", false) || !StrContains(classname, "tf_weapon_shotgun", false)))  //火箭
	{
		new Handle:itemOverride;
		if(iItemDefinitionIndex==127)  //Direct Hit 精确打击者
		{
			itemOverride=PrepareItemHandle(item, _, _, "114 ; 1 ; 179 ; 1.0 ; 57 ; 1 ; 76 ; 3 ; 2 ; 1.9 ; 16 ; 50", false);
				//114: Mini-crits targets launched airborne by explosions, grapple hooks or enemy attacks
		...
		{
			itemOverride=PrepareItemHandle(item, _, _, "114 ; 1 ; 57 ; 1 ; 76 ; 3 ; 2 ; 1.7 ; 16 ; 30", false);
				//114: Mini-crits targets launched airborne by explosions, grapple hooks or enemy attacks
				//57: 每秒回复3hP
				//76: 主武器子弹2倍
		}
	...
	//classname即武器的name设定 查阅https://wiki.alliedmods.net/Team_Fortress_2_Item_Definition_Indexes
	return Plugin_Continue;
}
...
public Action:CheckItems(Handle:timer, any:userid)
{
	...
	new playerBack=FindPlayerBack(client, 57);  //Razorback 电击背脊盾
	shield[client]=IsValidEntity(playerBack) ? playerBack : 0;
	/*if(IsValidEntity(FindPlayerBack(client, 57)))  //Cozy Camper
	{
		shield[client]=-1;
	}*/
	...
	new entity=-1;
	while((entity=FindEntityByClassname2(entity, "tf_wearable_demoshield"))!=-1)  //Demoshields 爆破手盾牌
	{
	...
	for(new client=1; client<=MaxClients; client++)
	{
		if(IsValidClient(client) && !IsBoss(client) && !(FF2flags[client] & FF2FLAG_CLASSTIMERDISABLED))
		{
			...
			if(!IsPlayerAlive(client))
			{
				...
				{
					FF2_ShowSyncHudText(client, rageHUD, "伤害: %d-%N 造成的伤害: %d", Damage[client], observer, Damage[observer]);
				}
				else
				{
					FF2_ShowSyncHudText(client, rageHUD, "伤害: %d", Damage[client]);
				}
				continue;
			}
			FF2_ShowSyncHudText(client, rageHUD, "伤害: %d", Damage[client]);
...
public Action:OnPlayerDeath(Handle:event, const String:eventName[], bool:dontBroadcast)
{
	...
	if(TF2_GetPlayerClass(client)==TFClass_Engineer && !(GetEventInt(event, "death_flags") & TF_DEATHFLAG_DEADRINGER)) //死亡后摧毁步哨
	{
		if (GetUserFlagBits(client) & ( ADMFLAG_CUSTOM2) == ( ADMFLAG_CUSTOM2))
		{
			// 拥有p(VIP)的步哨不会被摧毁
		}
		else
...
public Action:OnTakeDamage(client, &attacker, &inflictor, &Float:damage, &damagetype, &weapon, Float:damageForce[3], Float:damagePosition[3], damagecustom)
{
	...
	else
	{
		...
		if(boss!=-1)
		{
			if(attacker<=MaxClients)
			{
				...
				switch(index)
				{
					...
					case 61, 1006:  //Ambassador, Festive Ambassador 大使手枪伤害
					{
					...
					case 132, 266, 482, 1082:  //Eyelander, HHHH, Nessie's Nine Iron, Festive Eyelander, 苏格兰长剑
					{
						IncrementHeadCount(attacker);
					}
					case 214:  //Powerjack 千斤顶
					{
					...
					case 310:  //Warrior's Spirit 勇士之魂
					{
						new health=GetClientHealth(attacker);
						new newhealth=health+50;
						if(newhealth<=GetEntProp(attacker, Prop_Data, "m_iMaxHealth"))  //No overheal allowed
					...
					case 21, 208, 40, 215, 659, 741, 798, 807, 887, 896, 905, 914, 963, 972, 1146, 15005, 15017, 15030, 15034, 15049, 15054, 15066, 15067, 15068, 15089, 15090, 15115, 15141, 30474:	//火焰喷射器
					{
						new health=GetClientHealth(attacker);
					...
					case 357:  //Half-Zatoichi 座头市之刀
					{
						...
						new health=GetClientHealth(attacker);
						new max=GetEntProp(attacker, Prop_Data, "m_iMaxHealth");
						new newhealth=health+130;
						if(health<max+200)
						{
							if(newhealth>max+200)
							{
								newhealth=max+100;
							}
							SetEntityHealth(attacker, newhealth);
					...
					case 416:  //Market Gardener (courtesy of Chdata) 花园铲伤害
					{
				...
				if(bIsBackstab) //背刺伤害
				{
					damage=BossHealthMax[boss]*(LastBossIndex()+1)*BossLivesMax[boss]*(0.22-Stabbed[boss]/90)/6;
					damagetype|=DMG_CRIT;
					damagecustom=0;
					...
					else if(index==356)  //Conniver's Kunai 苦无
					{
						new health=GetClientHealth(attacker);
						if(health<300)
						{
					...
					else if(index==461)  //Big Earner 暴利杀手
					{
					...
					if(GetIndexOfWeaponSlot(attacker, TFWeaponSlot_Primary)==525)  //Diamondback 响尾蛇
					{
						SetEntProp(attacker, Prop_Send, "m_iRevengeCrits", GetEntProp(attacker, Prop_Send, "m_iRevengeCrits")+6);
					}
				...
				else if(bIsTelefrag)
				{
					...
					if(IsValidClient(teleowner) && teleowner!=attacker)
					{
						...
						{
							PrintHintText(teleowner, "传送杀助攻!  漂亮再来一次!");
						}
					}

					if(!(FF2flags[attacker] & FF2FLAG_HUDDISABLED))
					{
						PrintHintText(attacker, "传送杀! 你太厉害了!");
					}

					if(!(FF2flags[client] & FF2FLAG_HUDDISABLED))
					{
						PrintHintText(client, "传送杀! 小心传送装置!");
					}
					return Plugin_Changed;
				}
			}
		...
		else
		{
			...
			if(IsValidClient(client, false) && TF2_GetPlayerClass(client)==TFClass_Soldier)  //TODO: LOOK AT THIS 士兵的摔伤
			{
			...
			if(IsValidClient(client, false) && TF2_GetPlayerClass(client)==TFClass_DemoMan)  //TODO: LOOK AT THIS 爆破手的摔伤
			{
				if(damagetype & DMG_FALL)
				{
...
public Action:OnStomp(attacker, victim, &Float:damageMultiplier, &Float:damageBonus, &Float:JumpPower)
{
	...
	if(IsBoss(attacker))
	{
		...
		JumpPower=0.0;
		PrintHintText(victim, "诶呀!  小心你的头!");
		PrintHintText(attacker, "你居然把别人踩死了!");
		return Plugin_Changed;
	}
	else if(IsBoss(victim))
	{
		...
		PrintHintText(victim, "你被别人踩了一脚!");
		PrintHintText(attacker, "你成功踩了一脚BOSS!");
		return Plugin_Changed;
	}
	return Plugin_Continue;
}
